...
function Advertiser(accessoryInfo, port) {
...
Advertiser.prototype.startAdvertising = function() {
  ...
  for (ifName in ifaces) {
    if (ifName == 'en0' || ifName == 'eth0') {
      for (index in ifaces[ifName]) {
        if ('IPv4' == ifaces[ifName][index].family && ifaces[ifName][index].internal == false) {
          this._advertisement = mdns.createAdvertisement(mdns.tcp('hap'), this.accessoryInfo.port, {
            name: this.accessoryInfo.displayName,
            txtRecord: txtRecord,
            networkInterface: ifaces[ifName][index].address
          });
  ...
  if (!advertising) {
    console.log("Couldn't find IPv4 address to listen on");
  }
}
